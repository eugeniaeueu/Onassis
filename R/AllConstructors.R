...
CMdictionary <- function(inputFileOrDb = NULL, dictType = "OBO", outputDir = tempdir() ,synonymType = "EXACT", taxID = 0, outputDirOp=TRUE) {
    x <- new("CMdictionary")
...
CMoptions <- function(SearchStrategy='CONTIGUOUS_MATCH',
  ...
  opts@paramValueIndex <- as.character(as.vector(unique(options_combinations$paramValueIndex[
    which(options_combinations$SearchStrategy==SearchStrategy &
            ...
            options_combinations$Stemmer==Stemmer &
            options_combinations$Stopwords==StopWords &
            options_combinations$OrderIndependentLookup==OrderIndependentLookup &

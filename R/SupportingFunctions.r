...
loadEntities <- function(entityDirectory, deleteDir = TRUE) {
    ...
    if (file.access(entityDirectory, mode = 2) != 0) 
        stop(paste0("Unable to load data from ", entityDirectory))
    entityfiles <- list.files(entityDirectory, pattern = ".a1$")
    entity_table <- lapply(entityfiles, function(entityfile) {
        if (file.info(file.path(entityDirectory, entityfile))$size != 
            0) {
            fromerror = FALSE
            tryCatch({
                x <- read.table(file.path(entityDirectory, 
                  entityfile), header = FALSE, sep = "\t", 
                  as.is = TRUE)
                even <- seq(1, nrow(x), 2)
                odd <- seq(2, nrow(x), 2)
                ...
                if (grepl("_", term_id[1])) {
                  term_url <- as.character(sapply(term_id, 
                    function(single_term) {
                      splitted_id <- tail(strsplit(single_term, 
                        "_")[[1]], 2)
                      value <- splitted_id[2]
                      namespace <- splitted_id[1]
                      term_url <- paste0("http://purl.obolibrary.org/obo/", 
                        namespace, "_", value)
                    }))
                ...
                final <- unique(cbind(sample_id, term_id, 
                  term_name, term_url, matched_sentence))
                
                
            }, error = function(e) {
                final <- parseLineByLine(file.path(entityDirectory, 
    ...
    if (!is.null(unlist(entity_table))) {
        ...
        entity_df <- entity_df[which(sapply(entity_df$matched_sentence, 
            nchar) >= 3), ]
...
# This method is called from the other method when
# it is impossible to read the table and the file
# has to be read linbe by line
parseLineByLine <- function(cmfile, entityfile) {
    ...
    for (i in 1:length(lines)) {
        if (grepl("^T[0-9]", lines[i])) {
            ...
            if (grepl("_", term_id)) {
                splitted_id <- tail(strsplit(term_id, 
                  "_")[[1]], 2)
                ...
                term_id <- tail(strsplit(reference_sentence, 
                  split = " ")[[1]], 1)
                if (grepl("_", term_id)) {
                  splitted_id <- tail(strsplit(term_id, 
                    "_")[[1]], 2)
            ...
            table_row <- cbind(sample_id, term_id, 
                term_name, term_url, description_line)
            entity_table <- rbind(entity_table, table_row)
            
            
        } else {
            trimmed_line <- gsub("\\t", " ", lines[i])
            description_line <- paste0(description_line, 
                " ", trimmed_line)
        }
    }
    h <- close(conn)
    colnames(entity_table) <- c("sample_id", "term_id", 
        "term_name", "term_url", "matched_sentence")
    return(entity_table)
}


# This function writes an XML conceptmapper file
# from Org.xx.eg.db
...
prepareEntrezDictionary <- function(gene_symbols, gene_synonyms = NA) {
    ...
    dictionaryContent <- paste(dictionaryContent, "<synonym>", 
        sep = "\n")
    dictionary_total <- sapply(unique_symbols, function(symbol) {
        token_string <- paste0("<token id=\"", symbol, 
            "\" canonical =\"", as.character(gene_symbols[which(gene_symbols$gene_id == 
                symbol), 2]), "\">")
        if (is.data.frame(gene_synonyms)) {
            ...
            if (length(synonym_strings) > 0) {
                synonyms_list <- sapply(synonym_strings, 
                  function(synonym) {
                    synonym_tag <- paste0("<variant base=\"", 
                      synonym, "\"/>")
                  })
                synonyms_list <- paste(as.vector(synonyms_list), 
                  collapse = "\n")
                token_string <- paste(token_string, 
                  synonyms_list, collapse = "\n")
            }
            
        }
        token_string <- paste(token_string, "</token>", 
            collapse = "\n")
    })
    dictionary_total <- paste(dictionary_total, collapse = "\n")
    dictionaryContent <- paste(dictionaryContent, dictionary_total, 
        collapse = "\n")
    dictionaryContent <- paste(dictionaryContent, "</synonym>", 
        collapse = "\n")

...
# This function reads the .a1 files obtained from conceptmapper execution and converts them into a data frame
...
prepareEntrezDictionary <- function(gene_symbols, gene_synonyms=NA){
  ...
  dictionary_total <- sapply(unique_symbols, function(symbol){
    token_string <- paste0('<token id="', symbol,  '" canonical ="', as.character(gene_symbols[which(gene_symbols$gene_id==symbol),2]), '">')
    if(is.data.frame(gene_synonyms)){
      synonym_strings <- gene_synonyms$alias_symbol[which(gene_synonyms$gene_id==symbol)]

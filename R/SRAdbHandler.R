...
getSRAMetadata <- function(sra_con, library_strategy, library_source=NA, taxon_id=NA, center_name=NA){
  ...
  if(!is.na(taxon_id)){
    samples_query <- paste0("select sample_accession, description, sample_attribute, sample_url_link from sample where taxon_id='", taxon_id, "' and sample_accession IS NOT NULL")
  ...
  experiment_query <- paste0("select experiment_accession, center_name, title, sample_accession, sample_name, experiment_alias, library_strategy, library_layout, experiment_url_link, experiment_attribute from experiment where library_strategy='", library_strategy, "'")
  if(!is.na(library_source)){
    sources <- unique(dbGetQuery(sra_con, "select distinct library_strategy, library_source from experiment order by library_strategy")[,2])
    sources <- sources[which(!is.na(sources))]
    if(!library_source %in% sources)
      stop("Invalid library_source")
    else
      experiment_query <- paste0(experiment_query, " and library_source ='", library_source, "' ")
  }
    if(is.character(samples) & length(samples)>0)
    experiment_query <- paste0(experiment_query, " and sample_accession in ('", paste(unique(samples), collapse="','") , "')")
  if(!is.na(center_name)){
  ...
  experiment_df$experiment_attribute <- sapply(experiment_df$experiment_attribute, function(value){ gsub("||", "  ", value)})
  experiment_df$sample_attribute <- sapply(experiment_df$sample_attribute, function(value){ gsub("||", "  ", value)})

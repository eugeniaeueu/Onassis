...
In this section we provide a table with the details about Onassis performances during the execution of the typical annotation, computation of semantic similarities and comaprison tasks. For the main functions different input sizes and dictionary sizes were tested and reported. The code examples were run on a computer having a 3,5 GHz Intel Core i7 processor and 24 GB RAM. The table reports the name of the function, the elapsed time in seconds as reported by the R system.time function, the size of the object size in bytes as reported by object_size function, a description and the used code. We additionaly report the performances of the queries to SRA used to obtain the ChIP-seq dataset used in the vignette.

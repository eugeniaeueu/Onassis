...
Onassis can handle any type of text as input, but is particularly well suited for the analysis of the metadata from Gene Expression Omnibus (GEO).Indeed it provides the possibility to associate concepts from any OBO ontology to GEO metadata retrieved using `r BiocStyle::Biocpkg("GEOmetadb")`.
This represents a fundamental first step in the integrative analysis of the data from large scale repositories [@galeota2016ontology]. In general, any table or database (Sequence Read Archive (SRA) [@Zhu2013] or Cistrome [@Shengling2017] ) containing textual descriptions that can be easily imported in R as a data frame can be used as input for Onassis. 
In addition to the ontological concepts, the recognition of gene/protein symbols or epigenetic modifications can be highly relevant, especially for experiments directed to those specific factors or marks (such as ChIP-seq experiments).
...
The score module applies simple statistical tests to determine if biological signals from samples annotated with different concepts are significantly different.

# Installing additional libraries to run the examples in the vignette

To run Onassis Java (>= 1.8) is needed. Some of the optional functions, which will be described in the followign parts of the vignette, require additional libraries. 
For the analyses in this vignette please install following libraries:
...
Firstly, it is necessary to obtain a connection to the SQLite database. `connectToGEODB` returns a connection to the database given the path of the SQLite database file. If the latter is missing, it will be automatically downloaded into the current working directory. Because of the size of this file (0.5 GB), the results of the queries illustrated below are available into Onassis for the subsequent analyses illustrated in this document. Then, the `getGEOmetadata` function can be used to retrieve the metadata related to specific GEO samples, taking as minimal parameters the connection to the database and one of the experiment types available. Optionally it is possible to specify the organism and the platform. 
...
As specified before in this document, to correctly query GEOmetadb, it is necessary to download the corresponding database SQLite file, which occupies sever GB of disk space. Only for this vignette, meth_metadata was previously saved from the getGEOmetadata function and can be loaded from the Onassis package external data:  
...
The following code shows how to obtain SRA metadata of ChIP-Seq human samples: 
...
The previously saved sra_chip_seq can be retrieved in Onassis: 
...
* scores: a dataset of quantitative measurements (e.g. gene expression) associated to the samples annotated in entities and that will be separated in the different semantic sets identified in the annotation process.
...
As the method `annotateDF`, the column values will be pasted together before the annotation.
...
To slot `entities` of an Onassis object reports for each unique identifier the unique list of concepts found in the corresponding metadata. Term ids, term urls and term names will be reported and separated by a comma.
We usually refer to these unique lists as semantic sets.
...
The method `sim` cretes a matrix of the semantic similarities between the semantic sets of each couple of samples annotated in the entities slot of an Onassis object. 
...
Semantic sets with semantic similarities above a given threshold can be unified using the method `collapse`. This method, based on hierrchical clustering, unifies the similar semantic sets by concatenating their unique concepts. Term names and term urls in the `entities` slot will be updated accordingly. The method also adds a short_label column where, for each concept in the new semantic sets, the number of samples associated is also reported. At the end of the short label the total number of samples for that semantic set is also annotated.
After the collapse, the similarity matrix in the `similarity` slot will be consequently updated with the similarities of the new semantic sets.
...
In typical integrative analyses scenarios users could be interested in annotating concepts from different domains of interest. In this case, one possibility could be building a tailored application ontology including the concepts from different ontologies and their relationships. This method's disantavantages rely in the necessity to match and integrate the relationships from one ontology to the other. Alternatively, through Onassis, it is possible to annotate entities with another ontology and keep the different semantic sets separate.
In the following example ChIP-seq samples will be annotated with information about diseases. For the particular case of diseases Onassis provides a boolean variable `disease` for identifying samples explicitly annotated as `Healthy`. 
...
The method `mergeonassis` can be used to unify two Onassis objects, of which the first will contain semantic sets from a primary domain of interest. Samples in each semantic set will be further separated in semantic sets belonging to a secondary domain. For example, users could be interested in comparing different diseases (secondary domain) within a cell line (primary domain). 
...
The method `compare` can be used to compare the actual biological data from samples belonging to different semantic sets. 
To this end, a score matrix containing measurements for genomic units in the annotated samples is necessary.
The task of obtaining a correct score matrix is left to users. Anyway we suggest to make use of data from databases whose data representations are uniformed by standardazied analysis pipelines. This will avoid problems related to the differences in the normalization, sample sizes, platform types and genomic representations. For example CistromeDB [@Shengling2017], provides ChIP-seq data from GEO re-analyzed with a standardized pipeline. We decided to use peak files related to the previously annotated samples from Cistrome to show an example of how `compare` works. 
To obtain the peak files in BED format we relied on a batch download of the CistromeDB. Cistrome samples can be identified by GEO sample GSMs identifiers which were matched with those reported in the SRA ChIP-seq metadata table. We loaded the BED files in `Granges` objects and created a score matrix having as rows human promoter regions in chromosome 1 and as columns the sample identifiers. Each entry of the matrix contains the peak score value reported by Cistrome if there was a peak in the sample column, overlapping the region on the row. The score table can be retrieved from the vignette_data
...
The method `compare` can be used with any test function to compare scores, separated in semantic sets. For example we created a function that summarized, for each promoter region, the peak scores from different samples in one semantic set with the mean peak score and then calls the `t.test` function. 

```{r example_of_test_function, echo=TRUE, eval=TRUE}
personalized_t <- function(x, y){
		if(is.matrix(x))
			x <- apply(x, 1, mean)
		if(is.matrix(y))
			y <- apply(y, 1, mean)
...
disease_comparisons <- compare(cell_disease_onassis, score_matrix=as.matrix(score_matrix), by='col', fun_name='personalized_t')

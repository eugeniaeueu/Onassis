...
```
The function dict `CMdictionary` creates an instance of the class `CMdictionary` (by internally calling the method `buildDictionary`). 
...
Conceptmapper includes 7 different options controlling the annotation step. These are documented in detail in the documentaiton of the CMoption function. They can be listed through the `showCMoptions` function, and they can be set through the `CMoptions` function. The set options are thus stored in an object of class CMoptions that will be required for the subsequent step of annotation.  
...
similarity(obo, found_terms[1:2], found_terms[3:4])
...
heatmap.2(samples_results, density.info = "none", trace="none", main='Semantic similarity of annotated samples', margins=c(2,2))
```
